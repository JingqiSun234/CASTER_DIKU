...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Agentic Top One baseline.")
    ...
    parser.add_argument("--out", default="runs_v3_full/baselines/agentic/top_one")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Preload the local LLM before the timed selection loop; generation/control remains charged to update.",
    )
    parser.add_argument(
        "--dataset-key",
        action="append",
    ...
    print(
        "ok out={out} model=agentic_top_one selection_score_source={selection_score_source} "
        "selection_replay_used={selection_replay_used} forecast_rows={forecast_rows} expected_rows={expected_rows} "
        "metric_rows={metric_rows} llm_runtime_seconds={llm_runtime_seconds}".format(out=out, **timing)
    )
